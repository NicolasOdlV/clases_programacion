...
int fan_imprimir(Fantasma* pElemento)
{
	...
	{
		retorno = 0;
		printf("ID: %d- %s - %s - su valor es %.2f",pElemento->id, pElemento->campoStr1,pElemento->campoStr2,pElemento->campoFloat1);
	}
	return retorno;
}
...
int fan_imprimirArray(Fantasma* array,int limite)
{
	...
	if(array != NULL && limite > 0)
	{
		...
		for(i=0;i<limite;i++)
		{
			...
			{
				printf("\nID: %d - %s - %s - su campoFloat1 es %.2f",array[i].id,array[i].campoStr1,array[i].campoStr2,array[i].campoFloat1);
...
int fan_altaArray(Fantasma* array,int limite, int indice, int* id)
{
	...
	{
		if(	utn_getNombre(auxiliar.campoStr1, MACRO1_LEN, "\nNombre?", "\nNombre invalido",2) == 0 &&
			utn_getDni(auxiliar.campoStr2, MACRO2_LEN, "\nDNI?", "\nDNI invalido",2) == 0 &&
			utn_getNumeroFlotante(&auxiliar.campoFloat1,"\nAltura?","\nAltura invalida",1,5,2) == 0)
		{
			respuesta = 0;
...
/**
 * \brief Ordenar el array de fantasmas por campoStr1
 * \param array Array de fantasmas
 * \param limite Limite del array de fantasmas
 * \return Retorna el incice de la posicion vacia y -1 (ERROR)
 *
		...
		do
		{
			...
			for(i=0;i<limite-1;i++)
			{
				...
				if(	array[i].isEmpty == 0 &&
					array[i+1].isEmpty == 0 &&
					strncmp(array[i].campoStr1,array[i+1].campoStr1,MACRO1_LEN) > 0)
				{
					auxiliar = array[i];
					array[i]= array[i+1];
					array[i+1] = auxiliar;
